...
from models.labelembeddingvae import LabelEmbeddingVAE

sys.path.append('.')
sys.path.append('src')

argParser = argparse.ArgumentParser()
argParser.add_argument("-c", "--config", default="embedding_esm2_vae_v1", help="config name without .py extension")
...
def train_one_ep(fold_model, vae_model, dl, optimizer, criterion, scaler, binarymetrics, logs, device, log_metrics=True):
    """ Train for one epoch """
    fold_model.train()
    running_loss_trn = 0
    ...
    for cpu_data in tk0:
        ...
        with torch.no_grad():
            mu, logvar = vae_model.encode(data[1])
        ...
        if log_metrics:
            with torch.no_grad():
                probs = vae_model.decode(logits.detach())
            binarymetrics.accumulate_batch(probs.to(torch.float16), data[1])
            del probs
...
def evaluate(fold_model, vae_model, dl_val, criterion, binarymetrics, logs, device):
    ...
    for cpu_data in tk1:
        ...
        with torch.no_grad():
            logits = fold_model(data[0])
            loss_val += criterion(logits, z).item()
            probs = vae_model.decode(logits.detach())
...
def save_test_preds(fold_model, vae_model, dl_test, fold_out_dir, device):
...
def main(config, device:str, eval_every:int, metric_every:int):
    ...
    vae_model = LabelEmbeddingVAE(latent_dim=384)
    vae_model.eval()
    ...
    for fold, (train_index, test_index) in tkfold:
        ...
        fold_model = get_model(
            ...
            num_of_labels=384, # VAE latent space dim
            **CFG["model_kwargs"])
        ...
        for epoch in tkep:
            ...
            train_one_ep(fold_model, vae_model, dl, optimizer, 
            ...
            if (epoch % eval_every == 0) or epoch == CFG["epochs"]:
                val_probs, val_trues = evaluate(fold_model, vae_model, dl_val, criterion, binarymetrics, logs, device)
                ...
                if epoch == CFG["epochs"]:
                    ...
                    save_test_preds(fold_model, vae_model, dl_test, fold_out_dir, device)
...
if __name__ == "__main__":
    if not os.path.exists(vae_model_fn):
        print('Prepare target compression VAE model before training with this script!')
        print('Check target_compression notebook')
    else:
