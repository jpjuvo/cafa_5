...
ia_file = './input/cafa-5-protein-function-prediction/IA.txt'
...
def gt_parser(gt_file, ontologies):
    ...
    with open(gt_file) as f:
        for line in tqdm(f, position=2, leave=False, desc="Parsing GT"):
            line = line.strip().split()
            if line:
                p_id, term_id = line[:2]
...
def pred_parser(f, ontologies, gts, prop_mode, max_terms=None):
    ...
    for ns in gts:
        matrix[ns] = np.zeros(gts[ns].matrix.shape, dtype=np.float16)#'float')
        ids[ns] = {}
        for term in onts[ns].terms_dict:
            ns_dict[term] = ns


    for line in tqdm(f, total=len(f), position=2, leave=False, desc="Parsing pred"):
...
def compute_metrics(pred, gt, tau_arr, toi, toi_ia, ic_arr, n_cpu=0):
    ...
    if 1:
        ...
        # no-parallel
        metrics = compute_metrics_(tau_arr, g, pred, toi, n_gt)
    columns = ["cov", "pr", "rc"]
    ...
    if ic_arr is not None:
        ...
        if 1:
            ...
            # no-parallel
            metrics_ia = compute_metrics_w_(tau_arr, g, pred, toi_ia, n_gt, ic_arr)
